...
Portable validation requires every expected section, ordered macro record, export-local ID, scope, typed name/body/icon data, valid UTF-8, character identity field, category, tag, Favorite flag, and association reference. Unknown fields, duplicate structural keys, duplicate IDs/GUID records, unsupported versions/types/scopes, and broken references are rejected with a field path. The parser's 4 MiB input ceiling plus depth, value, and record limits bound archival strings without imposing current native macro limits. Errors never echo macro bodies into chat.

Portable/library validity and native-creation validity are separate contracts. Portable validation asks whether MacroStudio can safely understand and preserve format-v1 data; it does not apply the current client's macro-name, macro-body, capacity, or combat restrictions. Native content validation occurs only in Preview when an Account or enabled source-character record has no exact native destination and would require `CreateMacro`. The round-trip invariant is explicit: every valid portable format-v1 export produced by MacroStudio must be accepted by the format-v1 parser and portable-model validator in the same or a newer compatible MacroStudio version. A valid archival record may still be blocked from native creation on a particular client.

File icons in portable format v1 are structural integer values, matching Export's contract; path icons must remain nonempty strings. Whether a native-target record's icon is accepted for current creation remains a native repository/API concern. Capacity belongs to Preview/Apply, combat belongs to Apply, and source character identity remains portable context rather than native macro identity.
